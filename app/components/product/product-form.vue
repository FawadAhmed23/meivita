...
const iconsToInclude = computed(
  () => JSON.parse(props.product?.icons_to_include?.value || '[]')
);
...
<template>
  <div class="relative lg:sticky lg:top-[calc(var(--header-height)+1px)]">
    <div class="flex flex-col gap-5 w-full lg:md:max-w-lg lg:pt-20 lg:mx-auto">
      <ProductHeader :product="product" :current-variant="currentVariant" />
      <ProductColorOptions :product="product" :matching-colors="matchingColors" />
      <ProductSizeOptions :product="product" :variants="variants" :selected-size="selectedSize"
        @select-size="selectSize" />
      <ProductAddToCart :current-variant="currentVariant" :variants="variants" />
      <!-- <ProductDetails :description="product.descriptionHtml" :details="product.details?.value"
        :shipping="product.shipping?.value" /> -->
      <div v-html="product.descriptionHtml" />
      <div class="flex flex-wrap gap-5">
        <div v-for="icon in iconsToInclude" :key="icon.id">
          <NuxtImg :src="`/img/${icon}`" class="w-24 h-24" />
        </div>
      </div>
    </div>
  </div>
